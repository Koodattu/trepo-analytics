...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--db-path", type=Path, default=resolve_db_path(), help="SQLite database path.")
    ...
    export_parser.add_argument("--output", type=Path, default=resolve_export_path(), help="JSON export path.")
    ...
    rate_parser.add_argument(
        ...
        help=(
            ...
            f"Defaults to ${OPENAI_REASONING_EFFORT_ENV} or {DEFAULT_OPENAI_REASONING_EFFORT}."
        ),
    )
    rate_parser.add_argument(
        "--batch-size",
    ...
    update_parser = subparsers.add_parser(
        "update-new",
        help="Fetch newly appeared submissions, then rate every work still missing an interest rating.",
    )
    ...
    refresh_parser = subparsers.add_parser(
        "refresh-downloads",
        help="Refresh download counts for every work already stored in the database.",
    )
    refresh_parser.add_argument("--delay", type=float, default=DEFAULT_DELAY_SECONDS, help="Delay in seconds between requests.")
    refresh_parser.add_argument("--timeout", type=int, default=30, help="HTTP timeout in seconds.")
    refresh_parser.add_argument("--limit", type=int, default=None, help="Only refresh this many stored works.")
...
def main() -> None:
    ...
    if args.command == "update-new":
        ...
        print(f"Reasoning effort: {rating_result.reasoning_effort}")
    ...
    if args.command == "refresh-downloads":
        monthly_result = run_monthly_download_refresh(
            database=database,
            delay_seconds=args.delay,
            timeout=args.timeout,
            limit=args.limit,
        )
        result = monthly_result.refresh
        print(f"Resumed failed handles: {'yes' if monthly_result.resumed_failures else 'no'}")
        print(f"Works considered: {result.works_considered}")
        print(f"Download counts updated: {result.downloads_updated}")
        print(f"Failures: {result.failures}")
        if result.failures:
            if monthly_result.state_path is not None:
                print(f"Failed handles queued in: {monthly_result.state_path}")
            raise SystemExit(1)
        return
